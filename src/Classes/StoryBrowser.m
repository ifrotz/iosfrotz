...
- (id)init {
	...
	if (!vers || ![vers isEqualToString: @"1.5"]) {
...
-(void)setStoryDetails:(StoryInfo*)storyInfo {
	...
	if (storySplashPath && [[NSFileManager defaultManager] fileExistsAtPath:storySplashPath])
	    ...
	    BOOL isZblorb = ([[storyPath pathExtension] isEqualToString:@"zblorb"]);
	    if (isZblorb) {
		NSData *data = imageDataFromZBlorb(storyPath);
		if (data) {
