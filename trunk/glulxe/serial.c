...
//  unsigned char ch;
  ...
  /* If we're storing for the purpose of undo, we don't need to do any
     byte-swapping, because the result will only be used by this session. */
  if (!portable) {
    res = write_buffer(dest, gstack, stackptr);
    if (res)
      return res;
    return 0;
  }
...
//  unsigned char ch;
  ...
  if (!portable) {
    res = read_buffer(dest, gstack, stackptr);
    if (res)
      return res;
    return 0;
  }
  ...
  while (frameend != 0) {
    ...
    frm = Read4(gstack+(frameend-4));

    frm2 = frm;

    frlen = Read4(gstack+frm2);
    StkW4(frm2, frlen);
    frm2 += 4;
    locpos = Read4(gstack+frm2);
    StkW4(frm2, locpos);
    frm2 += 4;
    ...
    while (1) {
      unsigned char loctype, loccount;
      loctype = Read1(gstack+frm3);
      frm3 += 1;
      loccount = Read1(gstack+frm3);
      frm3 += 1;
