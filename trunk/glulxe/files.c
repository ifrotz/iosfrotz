...
int locate_gamefile(int isblorb)
{
  if (!isblorb) {
    ...
    if (err) {
      ginit_err = "This Blorb file seems to be invalid.";
      return FALSE;
    }
    ...
    if (err) {
      ginit_err = "This Blorb file does not contain an executable Glulx chunk.";
      return FALSE;
    }
    if (blorbres.chunktype != giblorb_make_id('G', 'L', 'U', 'L')) {
      ginit_err = "This Blorb file contains an executable chunk, but it is not a Glulx file.";
      return FALSE;
    }
