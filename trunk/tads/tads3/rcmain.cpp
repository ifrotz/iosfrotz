...
static int copy_file_bytes(osfildef *fpin, osfildef *fpout, ulong siz)
{
    ...
    while (siz != 0)
    {
        ...
        if (osfrb(fpin, copybuf, cursiz)
            || osfwb(fpout, (unsigned char*)copybuf, cursiz))
            return 1;
    }
                                ...
                                int link_mode)
{
    ...
    if (create_new)
    {
        ...
        fp = osfopwb((char*)image_fname, file_type);
        if (fp == 0)
        {
        ...
        if (osfwb(fp, (unsigned char*)VMIMAGE_SIG, sizeof(VMIMAGE_SIG)-1)
            || osfwb(fp, (unsigned char*)buf, 2 + 32 + 24 + 10))
        {
     ...
     */
    fp = osfoprwb((char*)image_fname, file_type);
    if (fp == 0)
    {
    ...
    if (osfwb(fp, (unsigned char*)buf, 10 + 2))
    {
         ...
         entry = entry->get_next())
    {
        ...
        if (link_mode)
        {
            ...
            /* write the block */
            if (osfwb(fp, (unsigned char*)buf, 1 + url_len + 1 + flen))
            {
        ...
        resfp = osfoprb((char*)entry->get_fname(), OSFTBIN);
        if (resfp == 0)
        {
        ...
        /* write the entry */
        if (osfwb(fp, (unsigned char*)buf, 9 + url_len))
        {
            disp_error(hostifc, "%.*s: error writing contents entry for "
                       "resource \"%.*s\"",
                       (int)OSFNMAX, image_fname,
    ...
    oswp4(buf, mres_size);
    if (osfwb(fp, (unsigned char*)buf, 4))
    {
    ...
    /* write the EOF block */
    if (osfwb(fp, (unsigned char *)buf, 10))
    {
