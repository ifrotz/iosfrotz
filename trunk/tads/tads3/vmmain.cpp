...
static char *file_param_to_abs_path(const char *param)
{
    ...
    // FROTZTADS -- do it again since Frotz has a parallel Saves folder to the Games folder we want to be reachable
    os_get_path_name(path, sizeof(path), path);
                      ...
                      CVmHostIfc *hostifc)
{
    ...
    else
    {
        ...
        if (!params.load_from_exe && !found_image && params.saved_state != 0)
        {
            ...
            save_fp = osfoprb((char*)params.saved_state, OSFTT3SAV);
            if (save_fp != 0)
            {
                                          ...
                                          char *fname, size_t fnamelen)
{
    ...
    osfildef *fp = osfoprb((char*)savefile, OSFTBIN);
...
static int vm_get_game_type_for_file(const char *filename)
{
    ...
    fp = osfoprb((char*)filename, OSFTBIN);
...
void vm_interface_report(CVmMainClientIfc *cli, const char *fname)
{
    ...
    osfildef *fp = osfopwt((char*)fname, OSFTTEXT);
    if (fp == 0)
    {
        cli->display_error(
