...
/*
Name
  mcm.c - memory cache manager
Function
  Manages the high-level memory cache
Notes
  The memory cache manager is a subsystem providing high-level
  object cache management.  Each object in a cache can be swapped
  out to disk or relocated when it is not locked, allowing the
  cache manager to keep memory unfragmented and overcommitted.
Modified
