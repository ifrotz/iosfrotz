#ifdef RCSID
static char RCSid[] =
    "$Header: d:/cvsroot/tads/TADS2/FIO.C,v 1.4 1999/07/11 00:46:29 MJRoberts Exp $";
#endif
...
Notes
  none
Modified
  04/11/99 CNebel        - Include appctx.h, not trd.h.
  10/23/97 CNebel        - removed now-obsolete Think C hack.
                   ...
                   appctxdef *appctx, char *argv0)
{
    ...
    for (;;)
    {
        ...
        else if (fioisrsc(buf, "FST"))
        {
            ...
            siz = endpos - curpos;
            if (siz && siz < OSMALMAX
                && (bufp = p = (uchar *)osmalloc((size_t)siz)) != 0)
            {
                uchar *p1;
                ulong  siz2;
                uint   sizcur;
