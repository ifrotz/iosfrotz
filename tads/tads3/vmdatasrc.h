...
public:
    ...
    /* read bytes - returns 0 on success, non-zero on EOF or error */
    virtual int read(void *buf, size_t len) { return osfrb(fp, (char*)buf, len); }

    /* read bytes - returns 0 on success, non-zero on EOF or error */
    virtual int readc(void *buf, size_t len) { return osfrbc(fp, (char*)buf, len); }

    /* read a line of text (fgets() semantics) */
    virtual char *read_line(char *buf, size_t len)
    {  osfgets(buf, len, fp); return buf; }

    /* write bytes - returns 0 on success, non-zero on error*/
    virtual int write(const void *buf, size_t len)
        { return osfwb(fp, (unsigned char*)buf, len); }
