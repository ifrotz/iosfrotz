...
static void os_banner_delete_all(osbanid_t banner) {
    if (!banner || !banner->valid)
        return;
    os_banner_delete_all(banner->children);
    os_banner_delete_all(banner->next);
    os_banner_delete(banner);
}

void os_banners_clear() {
    if (!os_banners)
        return;
    os_banner_delete_all(os_banners);
    os_banners = NULL;
}
...
void os_banner_set_size(void *banner_handle, int siz, int siz_units, int is_advisory)
{
    ...
    if (banner->method != gwinmeth || banner->size != gwinsize) {
...
int os_banner_getinfo(void *banner_handle, os_banner_info_t *info)
{
    ...
    glk_window_get_size(win, &(banner->cwidth), &(banner->cheight));
